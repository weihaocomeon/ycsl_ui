...
const inBrowser = true //typeof window !== 'undefined'
export default {
    ...
    events: {
      ...
      validator (events) {
        ...
        events.forEach((event, index) => {
          if (!event.date) {  //外部传入的event必须有日期 否则下面的错误
            console.error('传入的参数中必须制定date参数.未制定该参数的索引值为:' + index)
            validate = false
          }
        })
        return validate
      ...
      let dateObj = new Date()
       if (inBrowser) {
        ...
        return {
          ...
          params: {
              curYear: dateObj.getFullYear(), //获得现在的年
              curMonth: dateObj.getMonth(), //获得现在的月
              curDate: dateObj.getDate(), //获得现在的日
              curEventsDate: 'all'
          }
        }
      }
